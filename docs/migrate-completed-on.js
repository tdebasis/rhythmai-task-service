...
// Run this script with: mongosh rhythmai migrate-completed-on.js

print("Starting CompletedOn migration...");

// Count tasks that need migration
const needsMigration = db["rhythmai-tasks"].countDocuments({
  $or: [
...
if (needsMigration > 0) {
  ...
  const result1 = db["rhythmai-tasks"].updateMany(
    {
      completedAt: { $exists: true },
      completedDate: { $exists: true },
      completedOn: { $exists: false }
  ...
  const tasksWithOnlyTimestamp = db["rhythmai-tasks"].find({
    completedAt: { $exists: true },
  ...
  const tasksWithOnlyDate = db["rhythmai-tasks"].find({
    completedDate: { $exists: true },
...
// Final verification
const remaining = db["rhythmai-tasks"].countDocuments({
  $or: [
    { completedAt: { $exists: true } },
    { completedDate: { $exists: true } }
  ]
...
const migrated = db["rhythmai-tasks"].countDocuments({
